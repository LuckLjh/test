 ...
 * 通过邮件发送指定事件的抽象类。
 * <p/>
 * 参考 http://logback.qos.ch/manual/appenders.html#SMTPAppender
 *
 * @param <E>
 *            E
 *
...
public abstract class SMTPAppenderBase<E> extends AppenderBase<E> {
    ...
    // ~ 14 days
    static final long MAX_DELAY_BETWEEN_STATUS_MESSAGES = 1228800
    ...
    private Session lookupSessionInJNDI() {
        ...
        try {
            ...
            addError(
                "Failed to obtain javax.mail.Session from JNDI location [" + jndiLocation + "]");
    ...
    protected void append(final E eventObject) {
        ...
        if ((lastTrackerStatusPrint + delayBetweenStatusMessages) < now) {
            addInfo("SMTPAppender [" + name + "] is tracking [" + cbTracker.getComponentCount()
                + "] buffers");
    ...
    private List<InternetAddress> parseAddress(final E event) {
        ...
        for (int i = 0; i < len; i++) {
            try {
                ...
                addError("Could not parse email address for [" + toPatternLayoutList.get(i)
                    + "] for event [" + event + "]", e);
    ...
    protected void sendBuffer(final CyclicBuffer<E> cb, final E lastEventObject) {
        ...
        try {
            ...
            Transport transport = session
                .getTransport(session.getProperty("mail.transport.protocol"));
     ...
     *
     * @param jndiLocation jndiLocation
     * @since 1.0.6
     */
     ...
     * 
     * @param charsetEncoding charsetEncoding
     */
    ...
    class SenderRunnable implements Runnable {
        ...
        SenderRunnable(final CyclicBuffer<E> cyclicBuffer, final E entity) {
        ...
        @Override
        public void run() {
            sendBuffer(cyclicBuffer, entity);
        }
    }
}
