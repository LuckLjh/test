...
public class ArrivalNotice {
    ...
    public void process(DelegateTask delegateTask) {
        ...
        final List<BpmConfNotice> bpmConfNotices = ApplicationContextHelper
            ...
            .find("from BpmConfNotice where bpmConfNode.bpmConfBase.processDefinitionId=?0 "
                + " and bpmConfNode.code=?1", processDefinitionId, taskDefinitionKey);
    ...
    public void processArrival(DelegateTask delegateTask, BpmConfNotice bpmConfNotice) {
        ...
        logger.debug("启动人 : {}", delegateTask.getVariable("initator"));
        logger.debug("变量 : {}", delegateTask.getVariables());
        //
