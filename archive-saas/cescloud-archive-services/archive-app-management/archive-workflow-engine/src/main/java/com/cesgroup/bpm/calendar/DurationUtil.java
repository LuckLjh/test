...
/**
 * <li>org.activiti.engine.impl.bpmn.behavior.UserTaskActivityBehavior.java
 * 在处理task的duedate时使用了DueDateBusinessCalendar<br>
 * <li>org.activiti.engine.impl.cfg.ProcessEngineConfigurationImpl.java
 * 把BusinessCalendar的实现注册到BusinesssCalendarManager中<br>
 * <li>org.activiti.engine.impl.jobexecutor.TimerDeclarationImpl.java
 * 是从BusinessCalendarManager里根据type获取的<br>
 * <li>org.activiti.engine.impl.jobexecutor.TimerDeclarationType.java
 * 是对BusinessCalendar的类型进行的注册<br>
 * <li>org.activiti.engine.impl.persistence.entity.TimerEntity.java
 * 是从BusinessCalendarManager里获取的CycleBusinessCalendar<br>
 * <br>
 * R3/2011/2012 以R开头就是重复多次，R后面是重复次数，如果没有次数，默认无限循环<br>
 * 2011/2012 从2011开始，到2012截止<br>
 * 2011/P1D 从2011开始，一天后截止<br>
 * P1D/2012 从一天后开始，2012截止<br>
 * P1D 从一天后开始，无截止时间<br>
 * <br>
 * UserTaskActivityBehavior的场景，如果P1D，就是持续1天，如果2012，就是持续到2012<br>
 * TimerDeclarationImpl的场景，可能是duration也可能是dueDate，也可能是cycle，如果是cycle，
 * 保存时会把当前时间作为开始时间<br>
 * TimerEntity的场景，只有为cycle时，才重新进行计算下一次的触发时间<br>
 * 
 * @author 国栋
 *
 */
    ...
    public DurationUtil(String text, AdvancedBusinessCalendar businessCalendar) throws Exception {
        ...
        if (this.isDuration(expressions.get(0))) {
            ...
            if (this.isDuration(expressions.get(1))) {
                ...
                this.period = this.datatypeFactory
                    .newDuration(this.end.getTime() - this.start.getTime());
            }
        }
