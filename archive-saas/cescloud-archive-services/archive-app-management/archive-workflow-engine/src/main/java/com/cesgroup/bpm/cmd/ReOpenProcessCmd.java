...
public class ReOpenProcessCmd implements Command<Void> {
    ...
    public Void execute(CommandContext commandContext) {
        ...
        ExecutionEntity processInstance = this.createProcessInstance(
            historicProcessInstanceEntity.getId(), businessKey, initiator, processDefinition);
     ...
     * @return ExecutionEntity
     */
    public ExecutionEntity createProcessInstance(ProcessDefinitionEntity processDefinition,
        String id) {
        ActivityImpl initial = processDefinition.getInitial();

        if (initial == null) {
            throw new ActivitiException("不能启动流程实例，初始节点为空");
        }
