...
activitiModeler
  ...
  .run(['$rootScope', '$timeout', '$modal', '$translate', '$location', '$window', '$http', '$q',
        function($rootScope, $timeout, $modal, $translate, $location, $window, $http, $q) {
            ...
            function fetchModel(modelId,bpmModelId,fileName) {
                var modelUrl = KISBPM.URL.getModel(modelId,fileName);
                $http({method: 'GET', url: modelUrl}).
                    success(function (data, status, headers, config) {
                    	processCode = data.data.model.properties.process_id;
                        $rootScope.editor = new ORYX.Editor(data.data);
                        $rootScope.modelData = angular.fromJson(data.data);
                        $rootScope.editorFactory.resolve();
                    }).
                    error(function (data, status, headers, config) {
                    	console.log('装载模型出错 id ' + modelId + ' ' + data.msg);
                    });
            }
            ...
            $rootScope.$on('$includeContentLoaded', function (event) {
	            if (!$rootScope.editorInitialized) {
	                ...
	                var bpmModelId = EDITOR.UTIL.getParameterByName('bpmModelId');
	                var fileName = EDITOR.UTIL.getParameterByName('fileName');
	                
	                fetchModel(modelId,bpmModelId,fileName);
