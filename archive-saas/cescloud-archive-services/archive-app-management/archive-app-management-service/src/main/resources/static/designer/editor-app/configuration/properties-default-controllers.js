...
var KisBpmStringPropertyCtrl = [ '$scope', function ($scope) {
    ...
    $scope.inputBlurred = function() {
    	...
    	if ($scope.property.value) {
    		if($scope.property.key == "oryx-overrideid"){
    			$scope.property.value = $scope.property.value.replace(/[^\a-\z\A-\Z]/g,""); 
    		}else{
...
var KisBpmStringIntPropertyCtrl = [ '$scope', function ($scope) {
    ...
    $scope.inputBlurred = function() {
    	$scope.valueFlushed = true;
    	if ($scope.property.value) {
    		$scope.property.value = $scope.property.value.replace(/\D/g,'');
    		if($scope.property.value > 100){
    			$scope.property.value = 100;
    		};
    	}
        $scope.updatePropertyInModel($scope.property);
    };
...
var KisBpmDialogPropertyCtrl = [ '$scope', '$modal', function($scope, $modal) {

    var opts = {
        template:  'editor-app/configuration/properties/dialog-popup.html?version=' + Date.now(),
        scope: $scope
    };
