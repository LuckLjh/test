...
angular.module('activitiModeler').controller('ActivitiMessageDefinitionsPopupCtrl',
    ['$scope', '$q', '$translate', '$timeout', function ($scope, $q, $translate, $timeout) {
        ...
        $scope.listchange = function(){
        	$scope.selectedMessages[0].id = $scope.selectedMessages[0].id.replace(/[^\a-\z\A-\Z]/g,"");
        };
        ...
        $scope.save = function () {
            ...
            if ($scope.messageDefinitions.length > 0) {
                ...
                for(var i=0;i<$scope.property.value.length;i++){
                	if($scope.property.value[i].id == ""){
                    	jQuery('.errortextId').html('id不能为空！');
                    	return;
                    }else{
                    	jQuery('.errorText').html('');
                    }
                	if($scope.property.value[i].name == ""){
                    	jQuery('.errortextName').html('name不能为空！');
                    	return;
                    }else {
                    	jQuery('.errorText').html('');
                    }
