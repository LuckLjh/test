...
angular.module('activitiModeler').directive('onRepeatFinishedRender', function ($timeout) {
    return {
      restrict: 'A',
...
angular.module('activitiModeler')
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
        ...
        $scope.editorFactory.promise.then(function () {
            ...
            $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
                ...
                if (shapes && shapes.length > 0) {
                    ...
                    var selectedItem = {'title': '', 'properties': [] , 'description':'' };
        ...
        $scope.propertyClicked = function (index) {
        	var type = $scope.selectedItem.properties[index].key;
        	if(jQuery('#'+type).children('#opacity').hasClass('opacity')){
        		return;
        	}
    	...
    	$scope.$on("ngRepeatFinished", function (repeatFinishedEvent, element){
    		//repeat 加载完成之后执行
    		var typeScope = angular.element(document.querySelector('#oryx-multiinstance_type')).scope();
    		if(typeScope != undefined){
    			if(typeScope.property.value == "Parallel"){
    				jQuery('#oryx-multiinstance_condition #opacity').remove();
    				jQuery('#oryx-multiinstance_condition_hidden #opacity').remove();
            	}
    		}
    		/*var html = '<div id="opacity" class="opacity-hidden"></div>';
    		jQuery('#oryx-multiinstance_condition #opacity').prepend();*/
    		/*$http({ method: 'POST',
    	        ignoreErrors: true,
    		...
    		var callactivitycalledelementScope = angular.element(document.querySelector('#oryx-callactivitycalledelement')).scope();
    		if(callactivitycalledelementScope != undefined){
    			var callValue = callactivitycalledelementScope.property.value;
    			if(callValue != ""){
    				var url=location.href.substring(0,location.href.indexOf("widgets"));
    				//jQuery("#callactivitycalledelementValue").html("流程");
    				   $http({ method: 'POST',
    	    	        ignoreErrors: true,
    	    	        headers: {'Accept': 'application/json',
    	    	                  'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'},
    	    	        transformRequest: function () {
    	    	        	...
    	    	        	url: url+"/ces-delegate-manage/query-process-by-definitionCode?code="+callValue
    	    	        })

    	    	        .success(function (data, status, headers, config) {
    	    	        	jQuery("#callactivitycalledelementValue").html("名称:"+data.name);
    		...
    		//操作配置默认值
    		var operationsScope = angular.element(document.querySelector('#oryx-operations')).scope();
    		if(operationsScope != undefined){
    			if(operationsScope.property.value == ""){
    				operationsScope.property.value = {};
    				operationsScope.property.value.operation = {dialogcontent0:'submitTask'};
    				operationsScope.updatePropertyInModel(operationsScope.property);
    			}
    		}
...
jQuery('body').on('mouseover','.tooltip',function(){
	jQuery(this).children('.tooltipbox').show();
});
jQuery('body').on('mouseleave','.tooltip',function(){
	var that = jQuery(this);
	setTimeout(function(){
		that.children('.tooltipbox').hide();
	},1000);
	
});
jQuery('body').on('click','.tooltipcancel',function(){
	jQuery(this).parent('.tooltipbox').hide();
});