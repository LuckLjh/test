...
var KisBpmEventListenersPopupCtrl = [ '$scope', '$q', '$translate', '$timeout','$http',function($scope, $q, $translate, $timeout,$http) {
    ...
    $scope.keyup = function() {
        ...
        for(var i=0;i<$scope.property.value.eventListeners.length;i++){
        	var n= 0;
        	if($scope.property.value.eventListeners[i].className !="" ) n++;
        	if($scope.property.value.eventListeners[i].delegateExpression !="" ) n++;
        	if($scope.property.value.eventListeners[i].entityType != undefined && $scope.property.value.eventListeners[i].entityType !="" ) n++;
        	if(n<1){
        		html += '第'+(i+1)+'个事件：类，代理表达式，实体类型至少有一个必须填写！<br />'
        	}
        }
    ...
    $scope.save = function() {
        if ($scope.eventListeners.length > 0) {
        	...
        	for(var i=0;i<$scope.eventListeners.length;i++){
        		if($scope.eventListeners[i].events){
        			delete $scope.eventListeners[i].events;
        		}
        	}
    ...
    $scope.init = function() {
    	...
    	var url = "../workflow-version/listener/events?bpmModelCode=" + bpmModelCode + "&access_token=" + token; 
