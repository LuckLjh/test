$.component( "coral.autocomplete", $.coral.inputbase,{
	...
	castProperties : ["source","pageData","buttons","title","rootNode", "triggers","showRequiredMark","hideRequiredMark","itemattr"],
	options: {
...
/*	_createButtons: function() {
	...
	transTempData: function() {
		...
		if (this.options.source && this.options.render === "tree") {
			...
			convtree(this.options.source, this.options);
		}
		if (this.options.source && this.options.render !== "tree") {
			this.options.tempData = {};
			this.options.textField = this.options.textField || "text";
			this.options.valueField = this.options.valueField || "value";
			for(var k =0; k< this.options.source.length;k++){
				this.options.tempData[this.options.source[k][this.options.valueField]] = this.options.source[k];
			}
		}
	},
	...
	__response: function( content ) {
		...
		if ( !this.options.disabled && content && content.length && !this.cancelSearch ) {
			var length = content.length,
			    showNum = that.options.rowNum,
	            number = Math.ceil(length/showNum);
			if (number>1){
		...
		if ( this.options.multiLineLabel ) {
			...
			if ( $.trim( value ) == "" ) {
				...
				var texts = [],
					...
					cArr = value.split( "," );
				for ( var i=0; i<cArr.length; i++ ) {
					if(cArr[i] in this.options.tempData){
						texts.push( this.options.tempData[cArr[i]][textField] );
					}
				}
				if ( texts.length ){
