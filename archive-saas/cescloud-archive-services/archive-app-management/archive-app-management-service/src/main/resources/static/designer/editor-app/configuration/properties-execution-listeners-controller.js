...
var KisBpmExecutionListenersPopupCtrl = [ '$scope', '$q', '$translate', '$timeout','$http', function($scope, $q, $translate, $timeout, $http) {
	//解决ie9 箭头穿透的问题
    ...
    $scope.keyup = function() {
    	...
    	if($scope.property.value.executionListeners = null && $scope.property.value.executionListeners != undefined){
        	for(var i=0;i<$scope.property.value.executionListeners.length;i++){     	
        		var n= 0;
        		if($scope.property.value.executionListeners[i].className !="" ) n++;
        		if($scope.property.value.executionListeners[i].delegateExpression !="" ) n++;
        		if($scope.property.value.executionListeners[i].expression !="" ) n++;
        		if(n<1){
    ...
    $scope.init = function() {
    	...
    	var url = "../workflow-version/listener/executions?bpmModelCode=" + bpmModelCode + "&access_token=" + token; 
