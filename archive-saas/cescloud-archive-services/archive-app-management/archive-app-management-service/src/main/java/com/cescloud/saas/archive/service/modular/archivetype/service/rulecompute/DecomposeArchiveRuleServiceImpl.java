...
@Slf4j
@Service
@RuleHandler(FormStatusEnum.DECOMPOSE)
public class DecomposeArchiveRuleServiceImpl implements ArchiveRuleService {
	 ...
	 * （将卷内文件移到未组卷中，同时删除案卷）。
	 * （规则向上）
	 * 如果案卷有父档案即项目，则根据设置的求和、计数、求起止值、最大值、最小值、平均值规则更新父档案对应的字段值。
	...
	public Map<String, List<ColumnComputeRuleDTO>> handleComputeRule(String storageLocate,Long moduleId) throws ArchiveBusinessException {
		...
		if (StrUtil.isNotBlank(upStorageLocate)) {
			...
			if (CollectionUtil.isNotEmpty(list)) {
				List<ColumnComputeRuleDTO> columnComputeRuleDTOS = ruleComputeService.convertInnerRelation(FormStatusEnum.DECOMPOSE, list);
