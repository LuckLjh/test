...
@Slf4j
@Service
@RuleHandler(FormStatusEnum.DELETE)
public class DeleteArchiveRuleServiceImpl implements ArchiveRuleService {
	...
	/**
	 * （规则向上）根据设置的求和、计数、求起止值、最大值、最小值、平均值规则更新父档案对应的字段值
	...
	public Map<String, List<ColumnComputeRuleDTO>> handleComputeRule(String storageLocate,Long moduleId) throws ArchiveBusinessException {
		...
		if (StrUtil.isNotBlank(upStorageLocate)) {
			...
			if (CollectionUtil.isNotEmpty(list)) {
				List<ColumnComputeRuleDTO> columnComputeRuleDTOS = ruleComputeService.convertInnerRelation(FormStatusEnum.DELETE, list);
